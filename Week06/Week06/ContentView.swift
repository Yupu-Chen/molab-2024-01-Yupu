...
struct ContentView: View {
    ...
    func iconBarView(geo :GeometryProxy) -> some View {
        HStack{
            if !running && !custom {
    ...
    var pauseStartButton : some View {
        Button(action: {
            withAnimation {
    ...
    var standardTimeView : some View {
        Text(String(format:"%02i:%02i",
                    currentTimer/60,
                    currentTimer % 60)
        )
    ...
    func countDownView(geo :GeometryProxy) -> some View {
        HStack{
            Spacer(minLength: geo.size.width/20)
            withAnimation{
    ...
    var plusViewSub : some View {
        ExpandableButtonPanel(primaryItem: plusPrimary, secondaryItems: plusSecondary
        )
    }
    
    var plusPrimary : ExpandableButton {
        ExpandableButton(label: Image(systemName: "plus"), action: {
    ...
    var plusSecondary : [ExpandableButton] {
        [
            ExpandableButton(label: Image(systemName: "bed.double"), action: {
